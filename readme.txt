...
You must have WooCommerce installed (tested up to 3.0.8) and have an ActiveCampaign account to make use of this plugin.
...
4. Enter your ActiveCampaign URL and your ActiveCampaign API key;
...
If you are having problems syncing with ActiveCampaign then the first thing to check is for error messages in the debug log. To do this you must enable debug logging.
...
If you wish to clear the debug log then your site administrator can remove the file `/wp-content/uploads/wc-logs/wc-ac-hook*.log` or through the admin panel you now have the option to delete the log you are viewing at 'WooCommerce > System status > Logs'.
...
1. Keep (or set) the 'Signup to Marketing' option 'No' to supress the default checkbox
